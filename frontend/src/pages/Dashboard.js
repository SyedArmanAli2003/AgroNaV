      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "20px 16px" }}>
        ...
        {displayRecommendations.length > 0 && (
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
            <button
              onClick={() => {
                const topOutlets = displayRecommendations.slice(0, 5);
                const message = [
                  `📍 AgroNav Daily Plan — ${new Date().toLocaleDateString("en-IN")}`,
                  `Rep: ${authContext.user?.name || "Field Rep"}`,
                  "",
                  ...topOutlets.map((o, i) =>
                    `${i + 1}. ${o.retailer_name || o.name} (${o.label || (o.priority_score > 0.8 ? "HIGH" : o.priority_score > 0.6 ? "MEDIUM" : "LOW")})\n` +
                    `   📦 ${o.nba?.product_to_pitch || o.product_recommended || "Check stock"}\n` +
                    `   🎯 ${o.reasons?.[0] || "Priority visit"}`
                  ),
                  "",
                  "Generated by AgroNav AI",
                ].join("\n");
                window.open(`https://wa.me/?text=${encodeURIComponent(message)}`, "_blank");
              }}
              style={{
                display: "flex", alignItems: "center", gap: 6,
                background: "#25D366", border: "none", borderRadius: 99,
        ...
        {/* Low-bandwidth notice (2G / Data Saver) */}
        {isLowBandwidth() && !loading && (
          <div style={{
            ...
            Low connectivity detected — using lite mode (90% less data)
          </div>
        )}
